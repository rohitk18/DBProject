...
SortedDBFile::SortedDBFile(SortParams *sortParameters)
{
...
void SortedDBFile::SwitchToWriteMode()
{
    ...
    BigQ(*inputPipe, *outputPipe, *sortParams->myOrder, sortParams->runLength);
    useSameQueryOrderMaker = false;
    isWriteMode = true;
}
...
int SortedDBFile::GetNextFromDBFile(Record &fetchme, CNF &cnf, Record &literal)
{
    ...
    {
        cnf.GetCommonSortOrder(*sortParams->myOrder, *queryOrderMaker);
    }
...
void SortedDBFile::MergeCurrentFileAndBigQOutput()
{
    ...
    // Create Temp file as Heap and keep on adding records in sorted order.
    ...
    while (outputPipeNotFullyConsumed && currentFileNotFullyConsumed)
    {
        ...
        if (comparisonEngine.Compare(&currentFileRecord, &outputPipeRecord, sortParams->myOrder) < 1)
        {
...
int SortedDBFile::GetNextForSortedFile(Record &fetchme, CNF &cnf, Record &literal)
{
    if (queryOrderMaker->isEmpty())
    {
        ...
        {
            if (CheckForCNF(fetchme, cnf, literal))
                return 1;
        }
        return 0;
    }
    ...
    if (!useSameQueryOrderMaker)
    {
        ...
        if (doBinarySearch)
        {
            if (GetLengthInPages() - 1 <= currentlyBeingReadPageNumber + 1)
                return 0;
    ...
    while (GetRecordFromReadBufferPage(fetchme))
    {
        if (CheckForCNF(fetchme, cnf, literal))
            return 1;
        if (CheckForQuery(fetchme, literal))
            return 0;
    }
    return 0;
}
...
{
    return comparisonEngine.Compare(&literal, queryOrderMaker, &fetchme, sortParams->myOrder);
}
...
off_t SortedDBFile::BinarySearch(off_t low, off_t high, Record &literal)
{
    ...
    if (comparisonEngine.Compare(&literal, queryOrderMaker, &temp, sortParams->myOrder) > 0)
    {
